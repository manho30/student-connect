  ...
  <div id="errand-list-container">
        ...
        >
          ...
          <div class="space-y-1">
            <h3 class="font-bold text-slate-800 text-base">
              Need a favor on campus?
            </h3>

            <p
                class="text-xs text-slate-500 max-w-[200px] leading-relaxed"
            >
              Ask peers for help with deliveries, printing, or food.
            </p>
          </div>
    ...
    >
      ...
      <h3 class="text-lg font-bold text-slate-800 mb-1">
        No student errands found
      </h3>

      <p
          class="text-sm text-slate-500 max-w-md mx-auto mb-6"
      >
        Need someone to pick up food, collect a parcel, or print notes?
        Post an errand to ask fellow students!
      </p>
...
/**
 * Emits the accept event for an errand to the parent view.
 *
 * Permission checks are intentionally handled by ErrandCard for
 * presentation and by the backend for authoritative authorization.
 *
 * @param {string} errandId - Unique identifier of the errand.
 * @returns {void}
 */
function handleAccept(errandId) {
  emit('accept', errandId)
}

/**
 * Emits the complete event for an errand to the parent view.
 *
 * The assigned-helper permission is enforced by the card UI and,
 * most importantly, by the backend API.
 *
 * @param {string} errandId - Unique identifier of the errand.
 * @returns {void}
 */
function handleComplete(errandId) {
  emit('complete', errandId)
}

/**
 * Emits the select event when a student opens an errand.
 *
 * @param {string} errandId - Unique identifier of the selected errand.
 * @returns {void}
 */
function handleSelect(errandId) {
  emit('select', errandId)
}

/**
 * Opens the errand creation flow.
 *
 * @returns {void}
 */
